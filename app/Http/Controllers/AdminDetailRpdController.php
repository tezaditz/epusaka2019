<?php namespace App\Http\Controllers;
	...
	class AdminDetailRpdController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "detail_rpd";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			// $this->col[] = ["label"=>"M Rpd Id","name"=>"m_rpd_id","join"=>"m_rpd,id"];
			// $this->col[] = ["label"=>"Rkakl Id","name"=>"rkakl_id","join"=>"rkakl,id"];
			// $this->col[] = ["label"=>"No Mak Sys","name"=>"no_mak_sys"];
			$this->col[] = ["label"=>"Uraian","name"=>"uraian"];
			$this->col[] = ["label"=>"Sat","name"=>"sat"];
			$this->col[] = ["label"=>"Hargasat","name"=>"hargasat","callback_php"=>'number_format($row->hargasat , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Jumlah","name"=>"jumlah","callback_php"=>'number_format($row->jumlah , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Jan","name"=>"jan","callback_php"=>'number_format($row->jan , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Feb","name"=>"feb","callback_php"=>'number_format($row->feb , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Mar","name"=>"mar","callback_php"=>'number_format($row->mar , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Apr","name"=>"apr","callback_php"=>'number_format($row->apr , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Mei","name"=>"mei","callback_php"=>'number_format($row->mei , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Jun","name"=>"jun","callback_php"=>'number_format($row->jun , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Jul","name"=>"jul","callback_php"=>'number_format($row->jul , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Aug","name"=>"aug","callback_php"=>'number_format($row->aug , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Sep","name"=>"sep","callback_php"=>'number_format($row->sep , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Oct","name"=>"oct","callback_php"=>'number_format($row->oct , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Nov","name"=>"nov","callback_php"=>'number_format($row->nov , 0 , "," , ".")'];
			$this->col[] = ["label"=>"Dec","name"=>"dec","callback_php"=>'number_format($row->dec , 0 , "," , ".")'];
			# END COLUMNS DO NOT REMOVE THIS LINE

			# START FORM DO NOT REMOVE THIS LINE
			$this->form = [];
			$this->form[] = ['label'=>'M Rpd Id','name'=>'m_rpd_id','type'=>'select2','validation'=>'required|integer|min:0','width'=>'col-sm-10','datatable'=>'m_rpd,id'];
			...
			$this->form[] = ['label'=>'Mei','name'=>'mei','type'=>'money','validation'=>'required|integer|min:0','width'=>'col-sm-10'];
			...
			# OLD START FORM
			//$this->form = [];
			//$this->form[] = ["label"=>"M Rpd Id","name"=>"m_rpd_id","type"=>"select2","required"=>TRUE,"validation"=>"required|integer|min:0","datatable"=>"m_rpd,id"];
			...
			//$this->form[] = ["label"=>"Mei","name"=>"mei","type"=>"money","required"=>TRUE,"validation"=>"required|integer|min:0"];
	        ...
	        */
			...
			$this->addaction[] = ['label'=>'Edit','icon'=>'fa fa-pencil','color'=>'success','url'=>CRUDBooster::mainpath('edit').'/[id]','showIf'=>'[hargasat] != 0'];
	        ...
	        */
			// $this->load_js = array();
			$this->load_js[] = asset("js/monitoring/detail_rpd.js");
	    ...
	    */    
	    public function hook_row_index($column_index,&$column_value) {	        
			//Your code here
			if($column_index == 3 || $column_index == 4 || $column_index == 5 || $column_index == 6 || $column_index == 7 || $column_index == 8
			|| $column_index == 9 || $column_index == 10 || $column_index == 11 || $column_index == 12 || $column_index == 13 || $column_index == 14
			|| $column_index == 15 || $column_index == 16 ){
				if($column_value > 0) {
	    ...
	    */
		...
		public function getEdit($id)
		{
			if(!CRUDBooster::isUpdate() && $this->global_privilege==FALSE ) {    
			  ...
			  $data['row'] = DB::table('detail_rpd')->where('id',$id)->first();
			  
			  
			  //Please use cbView method instead view method from laravel
			  $this->cbView('backend.monitoring.rpd.edit',$data);
		}
